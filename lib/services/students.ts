  ...
  Object.fromEntries(
    Object.entries({
      ...
      school:
        student.school ?? null,
      exempted_months:
        student.exempted_months ?? student.exemptedMonths ?? [],
    }).filter(
 ...
 * كائنات Error في JavaScript تجعل الخاصية message غير قابلة للتعداد
 * (non-enumerable)، فطباعتها مباشرة عبر console.error قد تظهر في
 * بعض أدوات العرض كـ "{}" رغم أنها تحمل رسالة حقيقية.
 * نافذة أخطاء Next.js (خصوصاً مع Turbopack) تعرض أي وسيط (argument)
 ...
 * أولاً: أي صفوف تشير إلى سجلات material_deliveries الخاصة بالطالب
 ...
 * بعدها: كل الجداول المرتبطة مباشرة بالطالب عبر student_id.
 * أخيراً: صف الطالب نفسه من جدول students.
 * ==========================================================================
 */
...
// ====================================================================
// ==================== الإضافات الجديدة: الإعفاء الشهري ====================
// ====================================================================

/**
 * تحليل قيمة الأشهر المعفى منها من قاعدة البيانات
 * تدعم: مصفوفة، نص JSON، نص مفصول بفواصل
 */
...
/**
 * إضافة شهر إلى قائمة الإعفاءات الشهرية لطالب معين
 * @param studentId - معرف الطالب
 * @param monthKey - مفتاح الشهر بصيغة YYYY-MM (مثال: 2026-08)
 */
export async function addExemptedMonth(studentId: number, monthKey: string): Promise<Student> {
  ...
  // جلب قائمة الإعفاءات الحالية للطالب
  ...
  if (fetchError) {
    logSupabaseError('FETCH STUDENT FOR EXEMPT ERROR:', fetchError);
    throw fetchError;
  }

  const currentExempted = parseExemptedMonths(currentStudent?.exempted_months);

  // إضافة الشهر فقط إذا لم يكن موجوداً مسبقاً
  if (!currentExempted.includes(monthKey)) {
    currentExempted.push(monthKey);
  }

  // تحديث قاعدة البيانات
  const { data: updated, error: updateError } = await supabase
    .from('students')
    .update({ exempted_months: currentExempted })
    .eq('id', numericId)
    .select('*')
    .single();

  if (updateError) {
    logSupabaseError('ADD EXEMPTED MONTH ERROR:', updateError);
    throw updateError;
  }
...
/**
 * إزالة شهر من قائمة الإعفاءات الشهرية لطالب معين (إلغاء إعفاء)
 * @param studentId - معرف الطالب
 * @param monthKey - مفتاح الشهر بصيغة YYYY-MM (مثال: 2026-08)
 */
export async function removeExemptedMonth(studentId: number, monthKey: string): Promise<Student> {
  ...
  if (fetchError) {
    logSupabaseError('FETCH STUDENT FOR UNEXEMPT ERROR:', fetchError);
    throw fetchError;
  }

  const currentExempted = parseExemptedMonths(currentStudent?.exempted_months);
  const filtered = currentExempted.filter(m => m !== monthKey);

  // تحديث قاعدة البيانات
  const { data: updated, error: updateError } = await supabase
    .from('students')
    .update({ exempted_months: filtered })
    .eq('id', numericId)
    .select('*')
    .single();

  if (updateError) {
    logSupabaseError('REMOVE EXEMPTED MONTH ERROR:', updateError);
    throw updateError;
  }
...
/**
 * التحقق مما إذا كان طالب معفى من شهر معين
 * @param student - كائن الطالب
 * @param monthKey - مفتاح الشهر بصيغة YYYY-MM
 */
export function isMonthExempted(student: Student, monthKey: string): boolean {
  const exemptedMonths = student.exempted_months || student.exemptedMonths || [];
  return exemptedMonths.includes(monthKey);
}